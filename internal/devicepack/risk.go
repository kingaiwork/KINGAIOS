...
func minimumCapabilityRisk(c Capability) int {
	...
	for _, resource := range c.Resources {
		if candidate := resourceRiskFloor(resource); candidate > floor {
			floor = candidate
		}
	}
	return floor
...
func resourceRiskFloor(resource string) int {
	...
	case "sensor", "telemetry":
		return 0
	case "gpio", "i2c", "spi", "uart", "gpu", "npu", "accelerator", "camera":
		return 1
	case "microphone", "location", "network", "storage":
		return 2
	case "safety":
		return 3
	case "motor", "actuator", "relay", "power":
		return 4
	case "firmware", "boot", "flash":
		return 5
	default:
		return 0
	}
