...
func (m Manifest) Validate() error {
	...
	if strings.TrimSpace(m.Name) == "" || len(m.Name) > 160 || strings.TrimSpace(m.Version) == "" || len(m.Version) > 64 || strings.TrimSpace(m.Vendor) == "" || len(m.Vendor) > 120 { return errors.New("invalid name, version or vendor") }
	...
	for _, boardID := range m.BoardIDs { if strings.TrimSpace(boardID) == "" || len(boardID) > 160 || containsControl(boardID) { return errors.New("invalid board id") } }
	...
	for _, a := range m.Artifacts {
		if strings.TrimSpace(a.Name) == "" || len(a.Name) > 255 || containsControl(a.Name) || strings.ContainsAny(a.Name, "/\\") || a.Name == "." || a.Name == ".." { return errors.New("artifact name is invalid") }
		if _, ok := seenArtifacts[a.Name]; ok { return fmt.Errorf("duplicate artifact %q", a.Name) }; seenArtifacts[a.Name] = struct{}{}
		...
		hash, err := hex.DecodeString(a.SHA256); if err != nil || len(hash) != 32 || strings.ToLower(a.SHA256) != a.SHA256 { return fmt.Errorf("artifact %q has invalid sha256", a.Name) }
		...
		if a.Kind == "firmware" || a.Kind == "bootloader" || a.Kind == "driver" { if strings.TrimSpace(a.License) == "" || strings.TrimSpace(a.Source) == "" { return fmt.Errorf("artifact %q requires explicit license and source", a.Name) } }
	}
	if len(m.Capabilities) > 128 { return errors.New("too many device capabilities") }
	seenCapabilities := map[string]struct{}{}
	for _, capability := range m.Capabilities {
		if _, ok := seenCapabilities[capability.ID]; ok { return fmt.Errorf("duplicate capability %q", capability.ID) }; seenCapabilities[capability.ID] = struct{}{}
		if err := validateCapability(capability); err != nil { return fmt.Errorf("capability %q: %w", capability.ID, err) }
	}
	if len(m.Security.MinimumFirmwareVersion) > 128 || len(m.Security.Notes) > 2000 || containsUnsafeText(m.Security.MinimumFirmwareVersion) || containsUnsafeText(m.Security.Notes) { return errors.New("invalid security metadata") }
...
func CapabilityIDs(m Manifest) []string { ids := make([]string,0,len(m.Capabilities)); for _, capability := range m.Capabilities { ids=append(ids,capability.ID) }; sort.Strings(ids); return ids }
...
func validateCapability(c Capability) error {
	...
	rank := riskRank(c.Risk); if rank < 0 { return errors.New("risk must be L0 through L6") }
	if len(c.Resources) == 0 || len(c.Resources) > 32 || duplicateStrings(c.Resources) { return errors.New("invalid resource set") }
	for _, resource := range c.Resources { if !resourcePattern.MatchString(resource) || strings.ContainsAny(resource, "*?[]{};$`\\\n\r\x00") { return errors.New("resource contains wildcard, shell or control syntax") } }
	floor := minimumCapabilityRisk(c); if rank < floor { return fmt.Errorf("declared risk %s is below required L%d safety floor", c.Risk, floor) }
	if len(c.Description) > 500 || containsUnsafeText(c.Description) { return errors.New("invalid capability description") }
	if c.Operation != "read" && rank >= 3 && !c.ApprovalRequired { return errors.New("high-risk non-read capability requires approval") }
	if rank >= 5 && !c.ApprovalRequired { return errors.New("critical device capability requires approval") }
	return nil
}

func riskRank(risk string) int { if len(risk) != 2 || risk[0] != 'L' || risk[1] < '0' || risk[1] > '6' { return -1 }; return int(risk[1]-'0') }
func duplicateStrings(values []string) bool { seen:=make(map[string]struct{},len(values)); for _,value:=range values { if _,exists:=seen[value];exists{return true};seen[value]=struct{}{} };return false }
func containsControl(value string) bool { for _,r:=range value { if r<0x20||r==0x7f{return true} };return false }
func containsUnsafeText(value string) bool { for _,r:=range value { if r==0||r==0x7f||(r<0x20&&r!='\n'&&r!='\r'&&r!='\t'){return true} };return false }
