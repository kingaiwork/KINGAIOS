...
# The target root itself may inherit the CI workspace owner; normalize only the root inode.
chown 0:0 "$ROOT"
...
# Copy distribution overlays without preserving source checkout UID/GID. Keeping GitHub-runner ownership here would mutate system directories such as /etc inside the target image.
...
if [[ "$PROFILE" == "desktop" ]];then mkdir -p "$ROOT/usr/share/kingai" "$ROOT/usr/share/plasma/look-and-feel" "$ROOT/etc/xdg/autostart";cp -a --no-preserve=ownership desktop "$ROOT/usr/share/kingai/";cp -a --no-preserve=ownership desktop/look-and-feel/. "$ROOT/usr/share/plasma/look-and-feel/";install -Dm755 desktop/welcome/launch.sh "$ROOT/usr/lib/kingai/kingai-welcome";install -Dm644 desktop/welcome/kingai-welcome.desktop "$ROOT/etc/xdg/autostart/kingai-welcome.desktop";fi
# Never leak CI/build-host ownership into published KINGAI files copied from the repository.
