...
case "$PROFILE" in server|desktop|iot|recovery);;*)echo "invalid profile: $PROFILE" >&2;exit 2;;esac
case "$ARCH" in amd64|arm64);;*)echo "invalid arch: $ARCH" >&2;exit 2;;esac
if [[ "$PROFILE" == "recovery" && "$ARCH" != "amd64" ]]; then echo "recovery profile is currently reviewed only for amd64" >&2; exit 2; fi
...
if [[ "$PROFILE" == "desktop" ]];then
  ...
  cp -a --no-preserve=ownership desktop "$ROOT/usr/share/kingai/";cp -a --no-preserve=ownership desktop/look-and-feel/. "$ROOT/usr/share/plasma/look-and-feel/";cp -a --no-preserve=ownership desktop/plasmoids/org.kingai.agentcenter "$ROOT/usr/share/plasma/plasmoids/"
  install -Dm755 desktop/welcome/launch.sh "$ROOT/usr/lib/kingai/kingai-welcome";install -Dm644 desktop/welcome/kingai-welcome.desktop "$ROOT/etc/xdg/autostart/kingai-welcome.desktop"
  test -f "$ROOT/usr/lib/systemd/system/sddm.service" || { echo "sddm.service missing from desktop rootfs" >&2; exit 1; };ln -sfn /usr/lib/systemd/system/graphical.target "$ROOT/etc/systemd/system/default.target";ln -sfn /usr/lib/systemd/system/sddm.service "$ROOT/etc/systemd/system/display-manager.service"
fi
