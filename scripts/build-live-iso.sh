...
rm -rf "$WORK"
mkdir -p "$ISO_ROOT/casper" "$ISO_ROOT/boot/grub" "$ISO_ROOT/.disk" "$ISO_ROOT/sbom"
cp "$KERNEL" "$ISO_ROOT/casper/vmlinuz"
cp "$INITRD" "$ISO_ROOT/casper/initrd"
cp "$SBOM" "$ISO_ROOT/sbom/KINGAI-OS.spdx.json"
...
cp "$SBOM" "$ISO.spdx.json"
SIZE=$(stat -c '%s' "$ISO")
SBOM_SHA=$(sha256sum "$ISO.spdx.json" | awk '{print $1}')
...
print(json.dumps({
    ...
    "size_bytes":int(size),
    "sha256":sha,
    "sbom":{"artifact":os.path.basename(path)+".spdx.json","sha256":sbom_sha,"format":"SPDX-2.3"},
