...
func main(){
	...
	case "execute": execute(os.Args[2:])
	case "version": fmt.Printf("KINGAI Installer %s\n",version)
	default: usage();os.Exit(2)
	}
}

func usage(){fmt.Fprintln(os.Stderr,`usage: kingai-installer <list|plan|execute|version>
...
func list(){devs,err:=installer.Discover();if err!=nil{fail(err)};writeJSON(installer.CandidateDisks(devs))}

func plan(args []string){fs:=flag.NewFlagSet("plan",flag.ExitOnError);target:=fs.String("target","","target disk");profile:=fs.String("profile","server","distribution profile");_ = fs.Parse(args);devs,err:=installer.Discover();if err!=nil{fail(err)};p,err:=installer.BuildPlan(devs,*target,*profile);if err!=nil{fail(err)};writeJSON(p)}
...
func execute(args []string){
	...
	_ = fs.Parse(args)
	res,err:=installer.Execute(installer.ExecuteOptions{Target:*target,Profile:*profile,SourceRoot:*sourceRoot,StateKey:*stateKey,Confirmation:*confirm})
	if err!=nil{fail(err)}
	writeJSON(res)
}

func writeJSON(v any){enc:=json.NewEncoder(os.Stdout);enc.SetIndent("","  ");_ = enc.Encode(v)}
func fail(err error){fmt.Fprintln(os.Stderr,"installer failed:",err);os.Exit(1)}
