...
func init() {
	...
	for _, capability := range runtime.Capabilities() {
		...
		rules[capability.ID] = policy.Rule{
			Risk:             risk,
			ApprovalRequired: capability.ApprovalRequired || ownerOnly,
			OwnerOnly:        ownerOnly,
		}
	}
	if err := policy.SetRuntimeRules(rules); err != nil {
		log.Fatalf("device runtime policy registration failed: %v", err)
	}
	...
	if err := os.Setenv("KINGAI_EXECD_SOCKET", brokerSocket); err != nil {
		log.Fatalf("set device broker execution socket: %v", err)
	}
	if err := os.Remove(brokerSocket); err != nil && !errors.Is(err, os.ErrNotExist) {
		log.Fatalf("remove stale device broker socket: %v", err)
	}
	listener, err := net.Listen("unix", brokerSocket)
	if err != nil {
		log.Fatalf("listen on device broker socket: %v", err)
	}
	if err := os.Chmod(brokerSocket, 0o600); err != nil {
		_ = listener.Close()
		log.Fatalf("protect device broker socket: %v", err)
	}
	...
	mux.HandleFunc("/v1/execute", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			switch {
			case errors.Is(err, devicepack.ErrUnknownDeviceCapability):
				code = http.StatusNotFound
			case errors.Is(err, devicepack.ErrResourceNotDeclared), errors.Is(err, executor.ErrInvalidRequest):
				code = http.StatusForbidden
			case errors.Is(err, devicepack.ErrHandlerUnavailable):
				code = http.StatusServiceUnavailable
			}
			writeDeviceJSON(w, code, result)
			return
		}
...
func deviceRiskLevel(risk string) policy.RiskLevel {
	if len(risk) == 2 && risk[0] == 'L' && risk[1] >= '0' && risk[1] <= '6' {
		return policy.RiskLevel(risk[1] - '0')
	}
	return policy.RiskTrustRoot
}

func writeDeviceJSON(w http.ResponseWriter, code int, value any) {
