...
func main(){if len(os.Args)<2{usage();return};switch os.Args[1]{case"version":fmt.Printf("KINGAI OS %s\n",version);case"status":status(os.Args[2:]);case"doctor":doctor(os.Args[2:]);case"policy":policyCmd(os.Args[2:]);case"desktop":desktopCmd(os.Args[2:]);default:usage();os.Exit(2)}}
