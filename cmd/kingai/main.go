...
func status(args []string){
	...
	if err:=daemonJSON(http.MethodGet,"/v1/status",nil,&remote); err==nil {
		...
		fmt.Printf("System:       %v\nVersion:      %v\nArchitecture: %v\nPolicy:       %v\n",remote["name"],remote["version"],remote["architecture"],remote["policy"]); return
	}
...
func doctor(){
	...
	if err:=daemonJSON(http.MethodGet,"/healthz",nil,&map[string]any{}); err!=nil { fmt.Printf("[warn] kingaid: %v\n",err) } else { fmt.Println("[ok] kingaid") }
	checks:=[]struct{name,path string}{{"policy","/etc/kingai/policy.json"},{"system config","/etc/kingai/system.json"},{"agent config","/etc/kingai/agents.json"},{"model config","/etc/kingai/models.json"}}
	for _,c:=range checks { if _,err:=os.Stat(c.path); err==nil { fmt.Printf("[ok] %s\n",c.name) } else { fmt.Printf("[info] %s not installed at %s\n",c.name,c.path) } }
}

func policyCmd(args []string){
	if len(args)<2 || args[0]!="check" { usage(); os.Exit(2) }
	req:=policy.Request{Agent:"main",Capability:args[1]}; if len(args)>2 { req.Target=args[2] }
	...
	_=json.NewEncoder(os.Stdout).Encode(out); if !out.Allowed { os.Exit(3) }
}
...
func desktopCmd(args []string){
	...
	case "show": v,err:=desktop.Current(); if err!=nil { fmt.Fprintln(os.Stderr,err); os.Exit(1) }; if v=="" { fmt.Println("unselected") } else { fmt.Println(v) }
	case "set": if len(args)!=2 { usage(); os.Exit(2) }; if err:=desktop.Set(args[1],true); err!=nil { fmt.Fprintln(os.Stderr,err); os.Exit(1) }; fmt.Printf("desktop experience set to %s\n",args[1])
	default: usage(); os.Exit(2)
	}
}
...
func daemonJSON(method,path string,body any,out any) error {
	...
	tr:=&http.Transport{DialContext:func(ctx context.Context,_,_ string)(net.Conn,error){ return (&net.Dialer{}).DialContext(ctx,"unix","/run/kingai/kingaid.sock") }}; defer tr.CloseIdleConnections()
